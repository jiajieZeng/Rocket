...
void RpcDispatcher::dispatch(AbstractProtocol::s_ptr request, AbstractProtocol::s_ptr response, TcpConnection* connection) {
    ...
    if (method == NULL) {
        ERRORLOG("%s | method name[%s] not found in service[%s]", 
            req_protocol->m_req_id.c_str(), method_name.c_str(), service_name.c_str());
    ...
    INFOLOG("%s | req_id[%s], get rpc request[%s]", 
        req_protocol->m_req_id.c_str(), req_protocol->m_req_id.c_str(), req_msg->ShortDebugString().c_str());
